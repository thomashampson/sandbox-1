...
def main() -> int:
    ...
    close_issues_with_merged_pull_request(repository, milestone)
    return 0
...
def parse_arguments(argv: list) -> argparse.Namespace:
    ...
    parser.add_argument(
        "milestone", help="A milestone on the project. Checks all open issues for this milestone.")
...
def close_issues_with_merged_pull_request(repository: github.Repository.Repository,
    ...
    """
    For each open issue in the given milestone:
    ...
    """
    logging.info(f'Milestone "{milestone.title}" has {milestone.open_issues} open issues')
    ...
    open_milestone_issues = repository.get_issues(milestone=milestone, state='open')
    issues_to_close = find_issues_fixed_by_pull_requests(open_milestone_issues)
